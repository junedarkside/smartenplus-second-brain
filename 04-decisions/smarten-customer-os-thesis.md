...
- P1a field migrations (`source`/`origin`, `Conversation` FK) auto-leak through `'__all__'` serializers (`tickets/serializers.py`, `orders/serializers.py`) into admin-dashboard + frontend with no versioning → P1a is a **3-repo contract change**, not backend-internal. Pin serializer `fields` explicitly before shipping.
