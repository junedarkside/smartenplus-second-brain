...
| **P1b** | **Website chat widget** (**polls Django every 3s**, reuse `useQRPolling`) = customer channel for ALL types (direct + OTA). **CS Dashboard** (staff reply UI) subscribes to **Supabase Realtime** `cs.messages` channel (anon key, browser WebSocket, <1s push — no Django thread consumed). New Django `Conversation` + `Message` models (`sync_status` field tracks two-write consistency). Django→Celery writes message copy to Supabase `cs` schema (service_role key). **No NextAuth↔Supabase JWT bridge needed.** **Telegram = CS internal alert only** (new message/booking → staff group). **AWS SNS SMS** via `boto3` (same AWS account as SES) for trip reminders (day-before). Consent opt-in at first service-comms touchpoint → `marketing_consent` in Supabase ([[supabase-ota-booking-store]]). | 8-12 wks | after P2 (no infra gate) |
...
- P1a field migrations (`source`/`origin`, `Conversation` FK) auto-leak through `'__all__'` serializers. **3-agent scan found 103 `fields='__all__'` serializers** (r2 estimated 16 — actual blast radius much larger). Pin explicit fields on `TicketSerializer` (`tickets/serializers.py:55`) + `OrderSerializer` (`orders/serializers.py:94`) BEFORE any P1a migration. P1a is a **3-repo contract change**, not backend-internal.
...
- `@supabase/supabase-js` enters **admin-dashboard only** when P1b builds. `smartenplus-frontend` gets no Supabase client — widget polls Django.
