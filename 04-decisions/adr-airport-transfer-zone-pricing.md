...
### 4. Admin draws polygons — AD map page (`DrawingManager`).
Polygon vertices can't be hand-typed reliably → AD page with `@react-google-maps/api` `DrawingManager` (`libraries:['drawing']`): draw polygon → save `getPath().getArray()` vertices to zone API → reload as `<Polygon editable>` for vertex-drag edits. Add `@react-google-maps/api` to AD (same lib as FE). **Validity guards on save:** min 3 vertices, self-intersection check, priority for overlaps. Django admin = power-user fallback. Price still edited in existing AD contract pages (zone→contract pointer).
