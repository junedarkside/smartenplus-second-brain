...
| **TRIP-SEARCH-REDESIGN-R2** | Finish remaining ~60% of spec | **PLANNED 2026-06-14, backend-verified.** 11 small phases top→bottom of page: hero compact (P1 GATE), TrustChipRow extract, ConfidenceScore, RouteTimeline (timeline already serialized), card upgrade, BookedCounter `/10` fix, RecommendedTripCard, TravelInsight (FE rules), seats-available (P10 GATE: 1 BE method), SEO order. Branches: FE `feat/trip-decision-engine-r2`, BE `feat/seats-available`. On-time % CUT (no data). Next: P1 hero decision → P2. | [[trip-search-results-implementation-plan-2026-06-14]] Round 2 section |
