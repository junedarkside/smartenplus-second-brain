...
| **FE-22-REVIEW-REDIRECT** | Rate-review post-submit: `submit-review/[...slug].js:77-78` guards on `review?.data?.slug` but pushes `/rate-review/${review.data.booking_item_slug}`. If `booking_item_slug` null/≠ slug → user lands `/rate-review/undefined` immediately after submitting. Confirm API shape (is `booking_item_slug` always == `slug`?) then align guard+push field. Surfaced via rate-review audit re-verification 2026-06-24. | OPEN — P1 (deferred, API shape unverified) | `pages/rate-review/submit-review/[...slug].js:77-78` · [[rate-review-uxui-audit-2026-06-06-overview]] |
