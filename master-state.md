...
| # | Issue | Status | Where |
|---|-------|--------|-------|
| **REC-HYBRID-REGRESSION** | P0. Non-transport carts (spa/tour/ticket) get ZERO recommendations → widget hides. FE `CheckoutRelatedTrips.js:37` sends `recType='hybrid'`; backend `hybrid` runs only route-based finders (`find_similar`/`find_alternative`/`find_package`) which all early-return `[]` with no `trip.route`. `find_activity`/`find_nearby_activities` only reachable via `'activity'`. **Fix (decided):** backend `get_recommendations:692` — `hybrid` + no `trip.route` → `find_nearby_activities`. ~5 lines + BE deploy. Still data-gated on `primary_location` population. | OPEN #132 — found by /grill audit. Not yet coded. | `smartenplus-backend/products/services.py:692-709`, [[recommendation-engine-completion-roadmap]] |
