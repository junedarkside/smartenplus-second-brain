...
**Confirm with refinement.** Confirmed at the docs level (Round 1 R-domain already cited `[[08-archive/.originals/activities-location-search-bug.original]]` Debate 3, `ContractFormFields.js:184`, `operators/models.py:402-415` as three sources). From the customer frontend: zero evidence the "upselling" intent is realized. Zero evidence the "display" intent is realized. Only the "search-discoverability" intent is realized (the autocomplete dropdown).
