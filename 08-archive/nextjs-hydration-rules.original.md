...
`300` (number) = "refetch if cached data older than 300 seconds" — triggers immediately on cold mount with no cache. Use `true` to mean "refetch when args change". Use `false` to prevent refetch entirely. Never pass a number unless you explicitly want time-based stale cache refetch.

### PersistGate SSR Blocker

`PersistGate loading={null}` renders **null on the server**. Any component inside it — `DefaultSeo`, `Head`, `<Component>` — is suppressed during SSR. Result: `next-head-count="2"`, empty `<title>`, no OG tags in view-source.
