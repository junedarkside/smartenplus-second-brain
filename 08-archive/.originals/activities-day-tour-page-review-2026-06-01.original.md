...
## Summary
3-specialist code audit + grill + scrutinize of `/activities?category=DAY_TOUR`. 14 findings total. 3 Critical, 6 Major, 5 Minor. No live browser testing — source-only audit. Scrutinize corrected 4 wrong claims (FQ-4 symptom, UX-2 TRANSFER, FQ-1 title, DS-1 fix path). FQ-2 severity elevated. **Branch:** `260601-fix/activities-browse-audit` — ready for implementation.
...
- `confirm` field — secondary. Blocks availability check (`views.py:1180`), not listing. Not a browse filter.
- `stop_sale_dates` — date-level availability block only. Correct scope: availability-time, not listing. Stop sell is NOT the issue.
...
- User clicking "All" gets null category → API fires without `service_category` param → backend returns all categories → but on refresh URL has no `?category=` param so hook re-defaults to DAY_TOUR. State and URL disagree.
...
- MUI spacing unit 3 = 24px. `SPACING` in designSystem.js uses `md: '1.5rem'` (24px) — match is coincidental, not semantic.
...
- Mobile cards clamp tour name to 1 line. Most tour names are 40–70 chars. Single line truncates almost every title, destroying discoverability.
...
- Chips wrap on desktop (`md: 'wrap'`) so no scrollbar needed — correct. Mobile hides scrollbar entirely with no visual affordance for horizontal scroll. Users may not discover scrollable chip row.
...
- Grid column count matches — `xs=12 sm=6 md=4 lg=3 xl=3` identical in both skeleton (`DayTripList.js:65`) and actual cards (`DayTripList.js:105`). ~~"4-column layout" mismatch~~ — REFUTED by scrutinize, title was wrong.
- Actual issue: skeleton card internals (`DayTripList.js:35–61`) show "Operator", "Duration", "Key Features" skeleton rows — none rendered in actual `DayTripCard`. Perception mismatch on load.
...
- `useDayTripFilters.js:25–43` — `useEffect` then immediately fires `router.push` with default values → overwrites URL with defaults → URL loses actual query params on every page load.
- ⚠️ **Severity elevated by scrutinize:** mounted-ref fix alone insufficient. Root: `useState` initializer runs pre-hydration. Real fix: move URL reading into `useEffect` that fires post-hydration. Or use `useRouter` `isReady` guard before initializing state.
...
- OTA category pages benefit from `ItemList` schema listing top 3–5 results. Established pattern from [[trip-detail-page-review]] and [[homepage-seo-performance-deep-review-2026-05-21]].
...
- ~~"From THB 0"~~ — REFUTED by scrutinize. `PricingDisplay.js:34`: `if (!price || price <= 0)` → renders **"Price on Request"**, not "From THB 0". Guard already exists.
- Actual concern: contract has valid ratecards but price parse fails → silently shows "Price on Request" instead of real price. User sees wrong signal, not broken UI.
- **Fix:** log warning, not error. Price parse failure → `startingPrice = null` is cosmetically identical to 0 in PricingDisplay. Real fix: validate ratecard shape earlier. Lower priority than P2 — existing UX is acceptable.
...
UX-5 says "All" chip sets `null` and causes URL/state mismatch. FQ-2 says the useEffect fires spurious pushes. These compound: null category → URL has no `?category=` → on push hook re-initializes to DAY_TOUR default → "All" state lost immediately.
...
VD-1 wants to formalize 180px as token or change to 200px. FQ-1 wants skeleton to match card. These must be coordinated — whichever height wins, skeleton and card must use same value.
**Resolved:** Fix FQ-1 first (align skeleton to 180px current). Add token VD-1 after. No height change recommended — 200px would extend card beyond current grid without layout validation.
...
**Resolved:** Safe — CategoryFilter only drives UI selection, not API. URL params still pass through to API correctly. On-page chip just won't highlight for non-experience categories. Acceptable regression.
...
| FQ-1 | `DayTripList.js:35–61` | Skeleton anatomy doesn't match DayTripCard — shows fields that don't exist | Rewrite SkeletonCard to match: image 180px → 2 title lines → bullets → rating+price |
...
| Defer full COMPONENT_CONFIGS adoption | Yes — scoped DS-1 fix only | Full token refactor — high churn, low ROI for this sprint |
