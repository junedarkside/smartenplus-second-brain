...
## Summary
Django REST Framework API. Public, payment, cart/order, admin groups. Language via `Accept-Language` header (12 langs: en, th, zh, ja, ko, es, fr, de, ru, ar, ms, vi). Auth: token-based (API), session-based (admin).
...
### Tours & Contracts
- `GET /contract/` — list active contracts. Params: `?service_category=&operator=`.
- `GET /product-detail/{slug}/` — tour detail by slug.
- `GET /contract/{id}/availability/?date=YYYY-MM-DD&people=N` — availability check. ID not slug. Returns time slots + capacity.

### Auth
- `POST /api/auth/force-logout` — force logout, session invalidation. No auth.
...
- `POST /payments/order-charge/` → `ChargeOrderView` — initiate charge. Body: `{order_id, payment_method, amount, currency}`. Creates `GatewayCharge`. Idempotent via `IdempotencyKey`.
...
- `GET /payments/charge/{gateway_charge_id}/` → `ChargeDetailView` — charge details.
- `POST /payments/refund/` → `RefundCreateView` — create refund.
- `GET /payments/refund/{gateway_refund_id}/` → `RefundDetailView` — refund details.

---

## Cart & Order Endpoints

- `GET /api/orders/{id}/` — order details + 3DS polling fallback. Returns order with `GatewayCharge` status. PAID → `finalize_payment()`. PENDING → `reconcile_gateway_charge()` + `_sync_order_status()`.
...
- `DELETE /api/carts/{id}/cartitems/{item_id}/` — remove cart item.
