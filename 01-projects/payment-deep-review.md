...
- **M7 (BE)** Refunds: no over-refund guard in either recorder. Two live endpoints: `POST /admin-dashboard-charge/refund/` → `cards/views.py:53-140` (admin-dashboard-facing, mounted via `Smartenplus/urls.py:31`); `POST /payments/refund/` → `payments/views.py:322-351` (API-facing, mounted via `urls.py:36`). Neither validates cumulative sum. Order math in `cards/views.py:88-90` sums refunds across ALL charges for the order vs one charge's `total_amount` — can show misleading partial/full state. `GatewayCharge.status` never transitions to `refunded`/`partial_refunded`. Coupon `times_used` never decremented on refund. Fix priority: guard in both views — reject when `existing_refunds_total + request_amount > charge.amount`; set `GatewayCharge.status` accordingly.
  ...
  **Related operator-side mitigation:** [[not-suitable-for-section]] — operator-authored exclusion list (wheelchair/pregnant/age limits) displaces goodwill refund → policy-governed denial via pre-purchase disclosure.
...
3 Explore verifier agents cross-checked all 5 HIGHs + 18 MEDIUMs against ~1700 lines of vault payment/omise KB + read-only code spot-checks. **Result: 20 CONFIRMED · 2 REFINED (H1, M17) · 1 REFUTED (M4) · 1 KB inaccuracy (M8) · 12 KB gaps surfaced.** Full report: [[payment-deep-review-verification]].
...
Full list with all 12 in [[payment-deep-review-verification]] "KB Gaps Surfaced" section.
...
- BE H3 committed: `d7af0e9` on `smartenplus-backend` `fix/payment-deep-review`
- FE H4 committed: `a3c8c80` on `smartenplus-frontend` `fix/payment-deep-review`
- No push (manual PRs pending).

**Remaining:** Batch 2 (H2 + M10) → Batch 3 (H1 + M8) → Batch 4 (H5 + M5) → Batch 5 (M1–M3, M17, LOW sweep). See [[payment-implement-plan]] for full sequence.

---

## Related
[[booking-payment-e2e-audit-2026-06-11]] · [[payment-integration]] · [[payment-gateway-charge-architecture]] · [[payment-charge-service-layer]] · [[payment-status-enums]] · [[omise-webhook-security]] · [[payment-finalize-deep-dive]] · [[payment-qr-polling-mechanics]] · [[payment-celery-expiry-strategy]] · [[omise-api-reference]] · [[refund-flow]] · [[payment-checkout-5-principles]] · [[payment-deep-review-verification]] · [[payment-implement-plan]]
