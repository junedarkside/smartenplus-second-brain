...
All 3 PRs from [[frontend-architecture-audit]] landed on `develop`. 9 audit items resolved: 3 latent bugs (Formik render-prop extract, RTK Query auth branch, language store key) + 2 duplications (`bumpCartVersion` extraction, `createCart` single invalidation) + 1 dead-code cluster (5 items) + 1 repo-hygiene cluster (.backup/log files, backend material, archived design docs, .gitignore rules). Zero user-facing regressions. Date-helper consolidation (audit finding 6) deferred per audit ruling.
...
- [[frontend-architecture-audit]] — source audit
- [[booking-payment-e2e-audit-2026-06-11]] — earlier audit (4 bugs, all closed #94)
- [[frontend-test-infrastructure-audit]] — pre-existing test infra issues (mui-tel-input cascade) — not in scope
