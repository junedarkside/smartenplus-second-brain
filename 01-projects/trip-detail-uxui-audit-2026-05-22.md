...
### Methodology
Read `designSystem.js` (full). Read `TripDetailHero.js:55–60`, `TripDetailsAttribute.js:40–130`, `TripDetailContent.js:110–278`, `BestRateTable.js`, `TripDetailBooking.js:90–130`, `TimeLineDisplay.js`, `RelatedTripsSection.js:120–130`, `BookButton.js:260–291`. Grep `TYPOGRAPHY_SCALE` in trip detail → zero. Active in `components/order/`, `pages/bookings/`, `pages/blog/` (P1 isolated, not systemic).

### Findings

**VD1 — h1 mobile text-sm** | P1 | `TripDetailHero.js:57`

`text-sm md:text-3xl` = 12px mobile. Parent `text-lg` does NOT cascade to h1.
Fix: `text-base md:text-3xl`. **DONE.**

**VD2 — Section headings static text-lg** | P2 | Multiple

`TripDetailsImageAndMap.js:35`, `TripDetailContent.js:137,151,181,205`, `TripDetail2.js:153` — all fixed `text-lg`. HOMEPAGE_SECTION.heading = `text-xl`.
Fix: Deferred. Tag for typography pass.

**VD3 — TYPOGRAPHY_SCALE zero adoption in trip detail** | P1 | `components/itinerary/`, `pages/trips/detail/`

Predates token adoption. Other areas use tokens.
Fix: P1 — adopt next refactor pass.
...
Invalid Tailwind — no CSS generated.
Fix: `text-base` x3. **DONE.**

**VD6 — z-5 invalid z-index** | P1 | `TripDetailContent.js:114`

Tailwind scale: z-0, z-10, z-20, z-30, z-40, z-50. `z-5` → no CSS → sticky tabs lose stacking.
Fix: `z-10`. **DONE.**

**VD7 — Cards md:rounded-lg vs rounded-md standard** | P2 | Multiple

`md:rounded-lg`, no mobile rounding. ContentCard provides `rounded sm:rounded-lg`.
Fix: ContentCard wrapper. **DONE (via ContentCard).**

**VD8 — BookButton bgcolor #4267b3 secondary vs primary** | P2 | `BookButton.js:268`

#4267b3 = `COLORS.brand.secondary`. Book CTA should use primary (#3b5998).
Fix: `#3b5998`; hover `#2d4474`. **DONE.**

**VD9 — RelatedTripsSection + Reviews over-padding** | P2 | Multiple

`p-4 md:p-6` vs standard `p-2`. `rounded-lg` vs `rounded-md`.
Fix: Deferred for RelatedTripsSection. Reviews done.

**VD10 — Price text-xs in booking bar** | P2 | `TripDetailBooking.js:108`

Conversion info at 12px.
...
### Methodology
Read `[...slug].js:25–55` (imports + fallbacks), `:305–378` (render tree). Read `TripDetailHero.js`, `TripDetailBooking.js` (full), `BestRateTable.js`, `TripDetailContent.js:110–145`, `BookButton.js:260–295`, `TripDetail2.js:55–80`, `FeaturedImageHeaderSkeleton.js`. Journey: land → route → price → date → passengers → Book.

### Findings

**CF1 — Price lazy-loaded, no price in bar during hydration** | P1 | `[...slug].js:42–44`

`DynamicTripDetail2` fallback = `<div className="text-center p-4">Loading Details...</div>`. BestRateTable inside lazy block. Bar shows "Calculating..." until hydration. ISR mitigates repeat visits.
Fix: Deferred — needs `lowestRate` prop threading (medium effort).

**CF2 — Fixed bar obscures reviews; no pb-** | P1 | `[...slug].js:307`

Bar ~52px, no `pb-`. Last content hidden.
Fix: `pb-16`. **DONE.**

**CF3 — contractAvaliable=false empty state** | P1 | `TripDetailBooking.js:118`

Empty `<div>`. No messaging for advance_hr, stop_sale, inactive, out-of-range.
Fix: fallback `"Not available for this date"`. **DONE.**

**CF4 — BookButton disabled no aria** | P1 | `BookButton.js:277`

`disabled={true}` → `opacity-50`. No aria. SR: "Book Now, dimmed."
...
`cursor-pointer` but no chevron. Looks static.
Fix: `<ExpandMoreIcon fontSize="small" />`. **DONE.**

**CF6 — Calendar fallback CLS** | P1 | `[...slug].js:27`

Fallback ~36px → live ~80px = 44px CLS.
Fix: `min-h-[80px]` + matching margins. **DONE.**

**CF7 — TripDetail2 fallback CLS** | P1 | `[...slug].js:43`

Fallback ~48px → live ~600–800px = hard CLS.
Fix: `min-h-[600px]`. **DONE.**

**CF8 — lowestRate=null strands user** | P2 | `[...slug].js:279`

No nav link. Must browser back.
Fix: `<Link href="/trips">Search other trips →</Link>`. **DONE.**

**CF9 — Price not visible above fold mobile 375px** | P2 | Architecture

Hero `min-h-[200px]` + breadcrumb ~30px + calendar ~80px = ~310px before trip details. Bar shows price post-CSR.
Fix: Deferred — hero overlay redesign (medium effort).

**CF10 — Sticky tab z-index broken** | P1 | `TripDetailContent.js:114`

`z-5` → tabs behind calendar (`z-10`). Same root as VD6.
...
Per-component `mx-` breaks reuse on other pages.
...
32 issues. Root cause: trip detail = one-off, no shared abstractions. ContentCard wrapper pass resolves 8 issues in one commit (LS5, LS7, LS8, LS9, LS10, LS11, VD7).

Typography: 3 phantom/invalid classes — `text-md` (3x), `z-5` (1x), `text-slate-600` (off-palette). h1 `text-sm` at 12px mobile = highest-visibility fix.

Conversion: bar obscuring reviews (pb-16), empty booking state (CF3), lazy CLS (CF6, CF7), hidden affordance chevron (CF5).

Deferred (medium effort): CF1 price hydration, CF9 price above fold, VD9 RelatedTrips padding, VD3 TYPOGRAPHY_SCALE.
...
3. LS8/LS9/LS10 — 3 cards full-bleed → ContentCard `a31f12f`
