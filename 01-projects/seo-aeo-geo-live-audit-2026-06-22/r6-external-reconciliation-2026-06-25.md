...
| **C2** | `/destinations/[slug]` "Travel to undefined"; cause = `dest.name` typo | **TRUE bug / WRONG-CAUSE** | `pages/destinations/[slug].js:172` `arrivalStation=data?.station_name`; `getStaticProps` null-guards object (`:497`) but NOT missing field. No `dest.name` var exists. H1 `components/destinations/DestinationHeader.js:34` |
