...
| **[#374] verify remaining 4 unguarded sites live** | same day | Queried prod DB directly (local venv shares prod DB, confirmed via real data) rather than guessing endpoints. Found real polluted rows beyond the pier: `Location` id=1 (Hatyai), 6 `ImageGallery` rows (contracts 182/190/119 + 3 more). Verified at data layer through actual serializer classes: `RouteSerializer.get_arrival_location_image` on 5 real Hatyai routes → clean; `PopularExperienceSerializer.get_image` on 3 polluted contracts → clean. `PlaceSerializer`/`PlaceImageSerializer`'s `Image` model had zero polluted rows — nothing to break. A live-browser false alarm (stale `_next/image`/network-log cache in an already-open tab, filenames `23134535/4-2.jpg` + `04094239/36.jpg` showing cached 403s) triggered a DB-wide field scan for a possible 6th site — came back empty; user's own fresh-tab check confirmed no error, resolving it as a caching artifact, not a real gap. All 5 originally-identified sites confirmed fixed, no 6th found. |
...
## Closed — 2026-08-25 (session #352)

| Item | Closed | What shipped |
|------|--------|-------------|
| **LOCATIONOVERVIEW-HYDRATION-WARNING** | session #352 | About-section hydration error fixed. Moved `DOMPurify.sanitize()` from `LocationOverview.js` (client-only via `typeof window` guard) into `getServerSideProps` boundary in `pages/locations/[slug].js`. SSR + CSR now see identical sanitized HTML. Component became pure presentational (50→38 lines, no DOMPurify import). New `sanitizedDescription` prop wired through page. **Side-effect fix needed** — see LOCATIONS-PAGE-REDESIGN-COMMUNITY below. Commit `42aeb35e`. |
| **SELF-DESTINATION-CHIP** | session #352 | "Hatyai → Hatyai" self-chip gone. Filter applied once at page boundary in `pages/locations/[slug].js` (`filteredDestinations = useMemo(destinations.filter(d => d.name !== origin), [destinations, origin])`). Threads filtered list to LocationHero/DestinationSearch (dropdown), PopularRoutesSection (chips), useLocationDetailStructuredData (ItemList TouristTrip). Display-layer only — BE data unchanged (intra-hatyai airport transfer still legit product). Commit `d9a82b35`. |
| **LOCATIONS-FALLBACK-IMG** | session #352 | Hero image onError swap to default. `FeaturedImageHeader` already exposes `onImageError` prop (no new API). LocationHero.js adds `useState(imgSrc)` + `handleImageError` that swaps to `/smartenplus-transportation-booking-online.webp` (existing `bgDefault` from `useLocationDetailStructuredData.js`). Commit `46d1814d`. |
