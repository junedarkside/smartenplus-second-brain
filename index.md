...
- [[supabase-ota-booking-store]] — **KNOWLEDGE 2026-06-20.** Standalone Supabase store of Klook/12Go/Bookaway bookings + traveler profiles (email/name/phone + travel date + trip details + OTA ref + stable key), populated by parsing OTA confirmation emails (reliable, 80%+ coverage), accessed via Supabase REST/JS API. Ops island, disconnected from Django. **Overturns r2's "no traveler PII" blocker → conversion thesis reopened.** Owner-reported, not source-verified. Plan: read-sync to platform + add consent opt-in
