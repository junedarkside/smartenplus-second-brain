...
- [[cs-workflow-revised-2026-06-27]] — **ADR 2026-06-27 · ACCEPTED.** Post-team-meeting revision of CS-Centralization workflow. Stakeholder map, 3 trigger types (customer/admin-operator/OTA-initiated), Ticket state machine (pending→in_review→awaiting_ota_update→resolved/rejected/closed_no_action), booking completeness = derived, TripNotification model, Supabase webhook+Celery hybrid sync, frontend gaps + new components. 6 open questions logged. [[booking-command-centre-decision]] · [[cs-architecture-decision]]
