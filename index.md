...
- [[trip-search-results-implementation-plan-2026-06-14]] — **R1 SHIPPED · R2 IN PROGRESS 2026-06-14.** Round 1 merged to `develop`. Round 2 branch `feat/route-intelligence-hero` @ `496c74a`: hero redesign + filter entry fix + sort dedup + transport combo → MUI icon mini-cards DONE. Card-level phases (ConfidenceScore, RouteTimeline, RecommendedTripCard, BookedCounter, TravelInsight, seats-available) NOT started. Gate = user QA before merge to develop.
