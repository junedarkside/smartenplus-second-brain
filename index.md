...
- [[seo-sitemap-whole-site-audit-2026-06-11]] — **OPEN 2026-06-11.** 3-agent whole-site SEO+sitemap audit (infra/on-page/technical), code + live. Live checks blocked by Cloudflare 403 (= P0-1: verify Googlebot not WAF-blocked). 6 P0: fabricated review schema ×3 sources (incl. "TwoVit" copy-paste), sitemap ships ~20 private/dev URLs, `noindex` broken (next-seo has no `robots` prop) on checkout/orders/account, activities canonical malformed (`replace('/api')` eats `https://api`), no origin HTTP→HTTPS/www 301. 10 P1 incl. 480-line dead JSON-LD pipeline (`jsonLd` prop doesn't exist), schema URLs on API domain, invalid hreflang sitewide, server-sitemap xmlns `https://`. Soft-404 fix deliberately deferred to [[gsc-crawled-not-indexed-investigation-2026-06-05]] 3-phase plan. Fix order in note.
