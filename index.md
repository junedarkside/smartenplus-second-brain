...
- [[seo-aeo-geo-live-audit-2026-06-22/r3-synthesis|seo-aeo-geo-live-audit-2026-06-22]] — **AUDIT 2026-06-22 (LIVE PROD, whole-site).** SEO 7/10, AEO 5.5/10, GEO 1.5/10. Distinct from prior per-page code audits — measures what public web + AI engines actually receive. **3 decisive P0s:** (A) robots.txt blocks ALL AI crawlers (GPTBot/ClaudeBot/Google-Extended/CCBot/meta-externalagent Disallow /, Cloudflare-managed → invisible to ChatGPT/Claude/Gemini/Perplexity/Meta AI; fix = CF "AI Scrapers" toggle OFF, allowlist spec drafted); (B) 41 `/ref/article/*` in sitemap 404 (sitemap poison + wrong canonical path); (C) `/help` canonical→apex (chain), og:url=home, zero JSON-LD, client-only Q/A. Plus `og:locale:th_TH` hardcoded `_app.js:41` (English content), double-brand titles, route title 76c / activity title 131c, /help+/airport-transfer zero schema. Good: trip-detail+route-listing FAQ live in SSR (model pages), CWV floor LCP~1.7s/CLS 0/0 render-blocking, hero preload fetchpriority=high. 16-item P0-P2 list. Raw: r1-seo/r1-aeo/r1-geo. Read-only; specs drafted not applied.
