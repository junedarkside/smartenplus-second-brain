...
- [[adr-contract-soft-delete-2026-06-16]] — **NEW 2026-06-16.** Contract soft-delete via separate `is_deleted` flag (mirror ImageGallery). Hard-delete crashes on PROTECT FKs (BookingItem/CartItem). `is_deleted=True` also sets `is_actived=False` (belt-and-suspenders — every existing guard keys off `is_actived`); restore does NOT auto-reactivate. Frontend ZERO changes — backend public-queryset filter (`products/views.py:430`, detail `:845`) hides deleted. Admin gets Deleted chip + filter + Restore.
