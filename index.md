...
- [[seo-aeo-geo-live-audit-2026-06-22/r12-live-prod-2026-07-03|seo-aeo-geo-live-audit-2026-06-22 r12]] — **AUDIT 2026-07-03 (r12 LIVE PROD, post-r12 deploy).** 5-specialist audit. **All r12 P1s confirmed fixed** (s-maxage=3600, S3 preconnect, numberOfItems:6, currenciesAccepted:THB, koh-samui desc 140 chars). Scores: **SEO 9.1 · AEO 9.6 · GEO 9.0 · CWV 7.5 · SD 7.5**. Homepage now has 6 JSON-LD blocks (new TouristDestination ItemList). r13 P1: `/activities` hero lazy-loaded (no preload — LCP regression). r13 P2: destination pages missing BreadcrumbList + `@id` + stale priceValidUntil + seller name `"Smart En Plus"` inconsistency; Cloudflare not edge-caching HTML (CF-Cache-Status: DYNAMIC); twitter meta missing sitewide; sitemap has auth/utility pages. r13 P3: GEO — add OAI-SearchBot to robots.txt; Brotli unused (1-click CF toggle). Branch: `fix/seo-r13`. Canonical report: `seo/seo-aeo-geo-prod-2026-07-03.md`.
...
- [[command-centre-gap-audit]] — **AUDIT 2026-07-02.** 3-agent (BE/FE/Integration) failure-mode audit. 7 CRITICAL: PUT bypasses state machine, SLA machinery 100% dead (`calculate_sla_deadlines` zero callers — blockers-implementation note's SLA logic never wired to creation paths), guest chat token minted no-OTP, no EMAIL_BACKEND (all ticket emails silent no-op), resend-magic-link never sends, OTA resolve gate auto-bypassed w/o audit trail, admin-initiated direct tickets invisible to customer. Top claims leader-verified. Fix order: security triad → email foundation → SLA wiring → pagination → DB constraints. [[command-centre-ticket-booking-flow]]
...
- [[django-signals-ready-import-gotcha]] — **BUG PATTERN 2026-06-29.** `signals.py` handlers silently dead when `AppConfig.ready()` is absent. Django only imports signal handlers via `ready()`. Fix = 2 lines; all other apps follow this pattern. Case: `tickets/apps.py` — all customer emails never fired. [[cs-centralization-audit-2026-06-29]]
- [[genericfk-one-open-ticket-guard]] — **PATTERN 2026-06-29.** Guard against duplicate open tickets per booking when using GenericFK. Must query `(content_type_id, object_id)` pair + status in `Ticket.clean()`; call `full_clean()` in both create views. [[cs-centralization-audit-2026-06-29]]
- [[supabase-jwt-secret-location]] — **GOTCHA 2026-07-07.** Supabase JWT signing secret for PostgREST RLS: NOT the `sb_secret_...` key on API Keys page, NOT the ECC signing key. Location: Dashboard → Settings → JWT Keys → **Legacy JWT Secret** tab → Reveal. Also: `supabase.auth.setSession()` breaks with Django JWTs (GoTrue rejects non-auth.users tokens) — correct pattern: `createClient` with `accessToken` callback + `setSupabaseToken()` + `supabase.realtime.setAuth()`. Includes schema routing headers + sequence permission SQL.
...
- [[command-centre-direct-notify-redesign]] — **DECISION 2026-07-01 · BUILT (#205).** 3-debater architecture debate (Unify vs Isolate vs Hybrid) on letting command-centre notify DIRECT bookings. **Winner: Hybrid** — extract one shared `NotifyDialog` (`admin-dashboard/components/cs/NotifyDialog.jsx`, direct-only). Deciding factor (leader-verified): direct-ticket `content_object.id` already serialized (`tickets/serializers.py:48-50`, `BookingItemSerializer` `__all__`) → zero backend change. **Built** on BE `feat/cs-direct-bookings-tab` (`BookingItemListView`+`BookingItemTicketCreateView`+7 tests) + admin `feat/admin-direct-bookings-tab` (`csApi` hooks + `NotifyDialog` + `DirectBookingsTab` 3rd tab) — UNCOMMITTED, pending merge develop. Column "Service" (`contract_name`). OTA/[slug] retrofit deferred. [[cs-centralization-audit-2026-06-29]]
- [[cs-flat-dict-list-endpoint-pattern]] — **ATOM #205.** Staff-only `cs` list endpoints return flat dicts (no serializer) for command-centre tables; null-guarded derived-field helpers. Mirrors `OtaBookingListView`/`BookingItemListView`.
- [[shared-dialog-extraction-trigger]] — **ATOM #205.** Extract a duplicated inline dialog into a shared component only at a concrete 3rd call site + when the API layer already routes the discriminator (not before — YAGNI at 2 sites).
- [[booking-item-serializer-name-collision]] — **ATOM #206.** `BookingItemSerializer` defined in 4 apps (orders/bookings/tickets/dialogue). A view's `serializer_class` binds to the IMPORTED one, not the obvious one. Adding a field to the wrong class = silent no-op. Verify via view import + empirical shell/curl, not global class grep.
- [[cs-centralization-audit-2026-06-29]] — **AUDIT 2026-06-29 · ACTIVE.** Deep cross-layer re-audit (6 agents + 6 direct reads). NOT PRODUCTION-READY — 10 Tier-1 blockers found (signals dead, beat absent, resend dead token, no one-open-ticket guard, magic-link TTL broken, resolution side-effect absent, trip_id missing, closed_no_action unreachable, OTP full-site JWT, requested_value unbounded). Gap report 2026-06-27 SUPERSEDED. Recommended fixes scoped/reuse-first. [[cs-workflow-revised-2026-06-27]]
- [[cs-manual-test-flows-b7-e-2026-06-30]] — **TEST GUIDE 2026-06-30.** Click-by-click manual-test for remaining CS flows B-7/B-8/C/D/E (3-agent trace: FE-UI + admin-UI + BE/data, code-grounded). Bug found+fixed: `updateRequestStatus` dropped `resolution_note` (admin `9ac7089`). Prereqs incl. targeted OTA-seed shell (NOT destructive `seed_ota_fake_data`). [[cs-centralization-audit-2026-06-29]]
- [[chat-sender-session-bleed]] — **PATTERN 2026-06-30.** Dual-surface (customer widget + admin inbox) shared send endpoint: deriving sender from HTTP session bleeds (staff session on customer surface → misattributed `'cs'`). Fix: client sends role hint, server honors ONLY with ownership proof (guest token or conv owner) → spoof 403. "Never trust client" preserved via ownership gate. [[cs-centralization-audit-2026-06-29]]
- [[cs-centralization-gap-report-2026-06-27]] — ~~**GAP REPORT 2026-06-27 · SUPERSEDED 2026-06-29.**~~ Historical snapshot only. Stale: FE chat+OTP unreported; BE-B2/B4/B5 now shipped; 10 new criticals missed. See [[cs-centralization-audit-2026-06-29]].
...
- [[chat-supabase-impl-tasks]] — **PLAN 2026-07-05 · GATED.** Granular task card for the chat→Supabase migration: 7 phases, 23 small tasks, per-task model assignment (6 Haiku / 14 Sonnet / 3 Opus — Opus only for 2 security reviews + cutover go/no-go). Session-discipline rules: one phase = one session, explicit file paths, no codebase re-scan. Executes [[chat-supabase-migration-plan]] when [[cs-chat-supabase-offload]] activates.
- [[cs-chat-supabase-offload]] — **ADR DRAFT 2026-07-05 · PREPARED PATH.** Chat → Supabase Realtime full decoupling (customer widget + admin-dashboard staff inbox on Supabase; Django = batch archive via `sync_ota_bookings`-pattern Celery task + manual admin trigger). NOT rejected Option B — single write path kills R1/R2; R3 solved via Django-minted Supabase JWT + RLS. Activates at [[cs-architecture-decision]] flip trigger (>~30 req/s widget polling / staff-inbox build). Execution: [[chat-supabase-migration-plan]].
