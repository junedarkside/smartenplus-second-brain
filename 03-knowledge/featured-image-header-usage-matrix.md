...
## Summary
Cross-page comparison of `FeaturedImageHeader` component usage. 12 pages audited — 10 use component, 2 have no hero. Identifies inconsistencies in cinematic mode, min-height, search CTA, H1 typography.

## Context
`FeaturedImageHeader` (`components/UI/FeaturedImageHeader.js`) = shared hero component. Props: `title`, `imgUrl`, `children`, `actionButton`, `customMinHeight`, `blogTitle`, `onImageLoad`, `onImageError`, `imageLoaded`, `isCinematic`. Container defaults: `min-h-[200px] sm:min-h-[250px] md:min-h-[460px]`.
...
3. **Trip Detail `font-semibold`** — lighter vs `font-bold` everywhere else
4. **2 pages no hero** — Activities Browse (MUI Typography), Trip Detail by ID
...
7. **ColorThief dynamic gradient** — homepage only. Others use gray `rgb(156, 163, 175)` fallback
